...
<br>
Go has its own scheduler that allows many Goroutines to run on the same OS thread. It saves a significant amount of time on context switching. The scheduler also optimises by only running a Goroutine that has a non-empty channel with work to do. Only non-blocking Goroutines is assigned to the OS thread. Parallelism can be achieved as once a goroutine is assigned to an OS thread, the OS can distribute its threads' execution thorugh multiple cores.
